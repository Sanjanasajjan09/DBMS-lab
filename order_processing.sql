...
from Warehouses natural join Shipments 
...
--total number of orders by the customer and the last column is the average order amount 
--for that customer. (Use aggregate functions) 
...
BEGIN
	...
	set order_amt=(new.qty*(
		select distinct unitprice from Items 
		NATURAL JOIN OrderItems where item_id=new.item_id)) 
